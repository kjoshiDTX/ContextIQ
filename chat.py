...
def run_chat_interface(embedding_model, cross_encoder, vector_store, community_store, graph_store, llm):
    ...
    while True:
        try:
            ...
            if question.lower() == 'exit':
                break
            # community search
            print("looking for relavent community")
            question_embedding = embedding_model.encode(question).tolist()
            community_results = community_store.search(question_embedding, top_k=1)

            target_community_id = None
            if community_results:
                target_community_id = community_results[0][2].get("communityId")
                print(f"target community is {target_community_id}")
            ...
            if entities and target_community_id is not None:
                print(f"found entities {entities}. searching in community {target_community_id}")
...
def main():
    ...
    COMMUNITY_STORE_COLLECTION = "community_summaries"
    ...
    # connect to community vetor store
    community_store = MilvusVectorStore(embedding_dim=384, collection_name=COMMUNITY_STORE_COLLECTION)
    community_store.connect()
    ...
    # chat interface
    run_chat_interface(embedding_model, cross_encoder, vector_store, community_store, graph_store, llm)
